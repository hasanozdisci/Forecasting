...
socket.onmessage = function (event) {
  ...
  // if (lastPrice > stockObject.q)  then exchange rate is start with - else +
  btcExhange.innerHTML =
    lastPrice > stockObject.q
      ? "<b> -" + parseFloat(stockObject.q).toFixed(2) + "%"
      : "<b> +" + parseFloat(stockObject.q).toFixed(2) + "%";
...
//ETH
const socketForEth = new WebSocket(
...
socketForEth.onmessage = function (event) {
  ...
  etcExhange.innerHTML = lastPrice > stockObject.q
  ? "<b> -" + parseFloat(stockObject.q).toFixed(2) + "%"
  : "<b> +" + parseFloat(stockObject.q).toFixed(2) + "%";
...
// bnb coin price
const socketForBnb = new WebSocket(
...
socketForBnb.onmessage = function (event) {
  ...
  bnbExhange.innerHTML = lastPrice > stockObject.q
  ? "<b> -" + parseFloat(stockObject.q).toFixed(2) + "%"
  : "<b> +" + parseFloat(stockObject.q).toFixed(2) + "%";
...
// doge coin price
const socketForDoge = new WebSocket(
...
socketForDoge.onmessage = function (event) {
  ...
  dogeExhange.innerHTML = lastPrice > stockObject.q
  ? "<b> -" + parseFloat(stockObject.q).toFixed(2) + "%"
  : "<b> +" + parseFloat(stockObject.q).toFixed(2) + "%";
...
// tether coin price
const socketForTether = new WebSocket(
  "wss://stream.binance.com:9443/ws/tusdusdt@trade"
);
...
socketForTether.onmessage = function (event) {
  ...
  tetherExhange.innerHTML = lastPrice > stockObject.q
  ? "<b> -" + parseFloat(stockObject.q).toFixed(2) + "%"
  : "<b> +" + parseFloat(stockObject.q).toFixed(2) + "%";
